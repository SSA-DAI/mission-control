...
- **Busy detection di health verdict** (`src/lib/session-health.ts`) — `GatewaySessionInfo` kini membaca `hasActiveRun`/`activeRunIds` dari gateway `sessions.list` (sinyal turn aktif). `evaluateSessionHealth` menambah 2 kriteria rotasi: `hasActiveRun === true` → `session_busy:active_run`, dan status gateway di luar live set yang berarti busy (`processing`/`queued`/`pending`/`busy`/`blocked`) → `session_busy:<status>`. Sebelumnya hanya status non-`active`/`running` yang rotate → session yang turn-nya MASIH BERJALAN di-reuse → `EmbeddedAttemptSessionTakeoverError` (stall P009 15+ menit, butuh retry-dispatch manual).
- **Auto-recovery pasca-send** (`src/app/api/tasks/[id]/dispatch/route.ts`) — saat `chat.send` gagal dengan sinyal busy (marker takeover/prompt-lock/active-turn, detector murni `isBusySessionError`), dispatch OTOMATIS rotate ke session baru (`rotateToFreshSession`, runNumber+1) dan retry delivery SEKALI — tanpa menunggu operator trigger retry-dispatch. Activity `session_rotated` + event `session_rotated` dengan reason `busy_session:auto-recovery` (spec: alasan `busy_session`) tercatat di Activity tab + Live Feed. Path sukses di-refactor ke `finalizeDispatch()` agar bookkeeping (status/broadcast/events/activity) identik untuk dispatch normal dan retry.
- **Audit trail lebih lengkap** — `markSessionRotated` kini juga menyimpan `rotation_reason` di row lama (sebelumnya hanya row baru).
- **Regression tests** (+8, total 182/182 hijau) — `hasActiveRun=true` → unhealthy/rotate + runNumber+1 + key baru (`-r2-`); status `processing`/`queued` → rotate; idle tanpa active run → reuse (anti-churn); `isBusySessionError` marker detection; `rotateToFreshSession` (row lama rotated + reason tersimpan, row baru active).
